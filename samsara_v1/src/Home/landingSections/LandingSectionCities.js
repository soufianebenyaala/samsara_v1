      ...
      <div className={classes.cities_textcontainer}>
        <svg height="48" width="48">
          <g fill="none" fillRule="evenodd">
            <circle cx="24" cy="24" fill="#FFEFDD" r="24"></circle>
            <path
              d="M36.274 27.295a7.384 7.384 0 0 1-2.124 3.789c-1.283-7.59-6.17-7.496-6.17-14.49 0-1.588.178-3.122.484-4.594-6.848 1.606-12.028 9.064-12.028 18.035 0 .806.043 1.6.125 2.379a9.785 9.785 0 0 1-1.191-9.43A14.05 14.05 0 0 0 10 34.046c0 7.18 5.355 13.099 12.27 13.955-1.468-1.16-2.454-3.283-2.454-5.716 0-3.675 2.769-7.717 4.185-12.617 1.418 4.861 5.836 8.942 5.836 12.617 0 2.154-.775 4.063-1.97 5.279A14.058 14.058 0 0 0 38 34.045a13.99 13.99 0 0 0-1.726-6.75z"
              fill="#FFAC5F"
              fillRule="nonzero"
            ></path>
          </g>
        ...
        <Typography
          className={classes.cities_header}
          variant="h2"
          component="h2"
        >
          Our hottest cities.
        ...
        <Typography
          className={classes.cities_subHeader}
          variant="h4"
          component="div"
        >
      ...
      <div className={classes.link_container}>
        <div className={classes.cities_link_container}>
          {images.map((myImg, index) => (
            <div
              key={index}
              className={`${classes.cities_interactedBg} ${hidden[index]}`}
              style={{ backgroundImage: `url(${myImg})` }}
            />
          ))}
          <div className={classes.link_content}>
            <Typography className={classes.link_text} component="div">
            ...
            <div className={classes.popular_cities}>
              {cities.map((citie, index) => (
                <Button
                  key={index}
                  href={"/search/?searchIndex=" + citie[0]}
                  onMouseOver={() => handelHoverOnHover(index)}
                  onMouseOut={handelUnhover}
                  className={`${classes.popular_cities_link} ${Fade[index]}`}
                >
                  <span className={classes.popular_citieName}>{citie[0]}</span>
                  <span arial-hidden="true">{citie[1]}</span>
                </Button>
              ))}
            </div>
          </div>
