...
function avater({classes,currentUser,handelClickOpenSignIn,handelClickOpenSignUp}){
  if(currentUser){
    ...
    return(
      <div>
          <Button onClick={handelClickOpenSignIn} size="medium" className={`${classes.Login_button} ${classes.fontColor} ${classes.sm_hidden}`} >Log in</Button>
          <Button onClick={handelClickOpenSignUp} size="medium" variant="contained" className={`${classes.SignUp_button} ${classes.sm_hidden}`}>Sign up</Button>
      </div>
    );
  }
...
function Header(props) {
  ...
  return (
    <div className={classes.root}>
      <AppBar position={(props.position!=null)?props.position:'static'} color={props.color}>
        <Toolbar>
        <DrawerComponent 
